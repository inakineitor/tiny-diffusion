...
DATASAURUS_SHAPES: list[DatasaurusShape] = [
    "away",
    "bullseye",
    "circle",
    "dino",
...
NUM_DATASAURUS_SHAPES = len(DATASAURUS_SHAPES)
...
def dino_dataset(n: int = 8000, shapes: list[DatasaurusShape] | None = None):
    ...
    label_indices = torch.tensor([SHAPE_TO_INDEX[name] for name in df["dataset"].to_list()], dtype=torch.long)
